...
> https://www.zhihu.com/question/661744815/answer/3567269818  

### Dual Chunk Attention
DCA 通过将输入序列分块来减少计算复杂度，同时保留全局信息和局部信息。DCA 通常分为两个阶段：局部注意力和全局注意力。

在局部注意力阶段，输入序列被分成若干个块（chunks），每个块内的元素之间进行注意力计算。这种方式可以有效减少计算复杂度，因为注意力计算只在较小的块内进行。在全局注意力阶段，每个块的表示被用来计算全局注意力。具体来说，每个块的表示可以是该块内所有元素的平均值或最大值。然后，这些块表示之间进行注意力计算，从而捕捉全局信息。

> https://zhuanlan.zhihu.com/p/1911754084177904231  
实际使用的时候，如果token i和token j属于同一个块内，则q使用块内注意力的position id；如果属于相邻块，则q使用相邻注意力position id；如果超过相邻块，则q使用块间注意力的position id。
![alt text](dca.png)
...
## Qwen3（2025.04.29）【统一了think和no-think模式】
...
#### 第一步：Long-CoT冷启动
广泛的数学、代码、逻辑推理和一般STEM问题。**此阶段目标是建立基础推理模式，而非过度强调即时推理性能，确保模型潜力不受限制，为后续强化学习（RL）阶段提供更大的灵活性和改进空间。为有效实现这一目标，研究人员在准备阶段刻意控制训练样本数量和训练步骤。**

#### 第二步：推理强化学习
对于推理RL阶段的数据3995个(GRPO)

#### 第三步：思考模式融合
研究团队对推理强化学习模型进行了持续监督微调（SFT），在这个过程中将非思考模式整合到思考模型中。  

#### 第四步：通用RL
通用强化学习阶段主要是为了增加各种场景中的能力和稳定性，覆盖20多个任务
