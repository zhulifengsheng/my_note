...
## GRPO的问题：
1. 熵坍缩是其中一个最显著的障碍。在标准的强化学习训练中，随着模型收敛到高概率标记，策略会迅速变得确定性，**从而严重限制了对替代推理路径的探索**。这种过早的收敛阻碍了发现对于复杂数学推理至关重要的多样化解决方案策略。  
...
4. clip-higher：对clip的两个边界分别设置不同的 $\epsilon$ 来控制，其中上边界是一个较大的值来鼓励探索，新策略token的允许概率会更大。
